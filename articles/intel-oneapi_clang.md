...
自分のマシンなら良いですが，共用のマシンにインストールするのは今すぐ考え直してください．
少なくともbasekitやhpckitを入れた人は既に死んでます．理由について書いていきます．

この記事は以下2つの記事の続きです．何のためにインストール記事なんて書いたかって，この記事をちゃんと書きたかったからです．
...
- oneAPIからDPC++というLLVMベースのC++コンパイラが入りました．
    - このLLVMは通常のものではなく，[IntelによるLLVMのフォーク品](https://github.com/intel/llvm) (intel/LLVM)です
    - これはiccを入れると強制でくっついてきます ([前回の記事][2])
- intel/LLVMはiccやMKLが使われるものが含まれます
    - [余談] つまり従来までの有償iccと同じ系譜のものなのか，よく分かりません
- intel/LLVMが強制的に入ることにより，通常のLLVMと環境変数やライブラリがぶつかります
    - どちらかが死にます．共存できません
    - 共用サーバなどに入れるべきではないでしょう
...
そう．intel/LLVMのもつclangにパスが通され， `/usr/bin/clang` はこっそり外されました．
